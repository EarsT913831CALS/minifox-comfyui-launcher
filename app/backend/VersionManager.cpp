      ...
      m_configuration(configuration)
{
    ...
    m_repositoryWatchDebounce.setSingleShot(true);
    m_repositoryWatchDebounce.setInterval(400);
    connect(&m_repositoryWatchDebounce, &QTimer::timeout, this, [this] {
        if (!m_busy) loadLocalState();
    });
    connect(&m_repositoryWatch, &QFileSystemWatcher::fileChanged, this, [this](const QString &) {
        updateRepositoryWatch();
        m_repositoryWatchDebounce.start();
    });
    connect(&m_repositoryWatch, &QFileSystemWatcher::directoryChanged, this, [this](const QString &) {
        updateRepositoryWatch();
        m_repositoryWatchDebounce.start();
    });
...
void VersionManager::updateRepositoryWatch()
{
    const QStringList files = m_repositoryWatch.files();
    if (!files.isEmpty()) m_repositoryWatch.removePaths(files);
    const QStringList directories = m_repositoryWatch.directories();
    if (!directories.isEmpty()) m_repositoryWatch.removePaths(directories);

    const QDir gitDir(QDir(m_comfyRoot).filePath(QStringLiteral(".git")));
    const QString head = gitDir.filePath(QStringLiteral("HEAD"));
    if (QFileInfo::exists(head)) {
        m_repositoryWatch.addPath(head);
    }
    const QString refsHeads = gitDir.filePath(QStringLiteral("refs/heads"));
    if (QFileInfo::exists(refsHeads)) {
        m_repositoryWatch.addPath(refsHeads);
    }
}
